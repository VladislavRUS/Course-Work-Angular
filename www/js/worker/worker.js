    ...
    switch (action) {
        ...
        case 'graph': {
            ...
            try {
                ...
                postMessage({action: 'graph', path: null, matrix: clustered});
                console.log(exc);
            }
        ...
        case 'shortest': {
            ...
            var path = graph.findShortestPath(event.data.matrix);

            postMessage({action: 'shortest', path: path});
            break;
        }
    }
};
...
function getHoles(path) {
    var holes = [], next = true;

    for (var i = 0; i < path.length; i++) {
        if (path[i].color == 'white' && next) {
            holes.push({ length: 1 });
            next = false;

        } else if (path[i].color == 'white' && !next) {
            holes[holes.length - 1].length += 1;

        } else if (path[i].color !== 'white') {
            next = true;
        }
    }
